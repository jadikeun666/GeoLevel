...
### 🔄 In Progress
- **Map Feature** (visualisasi peta survei) — spesifikasi lengkap di `docs/map.md`
  - ✅ Fase 1 (tabel `survey_points`, model, controller, routes, 11 feature test) — selesai
  - 🔄 Fase 2 (maplibre-gl terpasang, `SurveyMap.vue` render marker, tab "Peta" di `Show.vue`) — sebagian: polyline, popup, network legs overlay, `LocationPicker.vue`, `MapLegend.vue` belum
  - ⏳ Fase 3 (`GpxImportModal.vue`, mini-map di `Projects/Index.vue`) — belum mulai

### ⏳ Not Started
- Lanjutan Map Feature Fase 2 akhir + Fase 3 (lihat `docs/map.md`)
...
penyebabnya — ternyata itu jalan buntu, bukan akar masalah.

### CSS MapLibre dimuat via CDN, bukan lewat Vite (KEPUTUSAN DESAIN)
